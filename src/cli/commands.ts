...
const CONFIG_KEYS: Record<string, { type: 'string' | 'number' | 'boolean' | 'number|null' | 'string|null' | 'string[]'; help: string }> = {
  ...
  'mirror.warnBytes': { type: 'number', help: 'mirror: warn when repo exceeds this many bytes' },
  'mirror.pauseBytes': { type: 'number', help: 'mirror: pause mirroring at this repo size (MIRROR PAUSED / REPOSITORY_CAPACITY)' },
  'mirror.maxFileBytes': { type: 'number', help: 'mirror: never commit files larger than this (GitHub 100MB hard limit aware)' },
  'mirror.maxPagesPerProvider': { type: 'number', help: 'mirror: discovery page bound per provider' },
  'mirror.storePreviews': { type: 'boolean', help: 'mirror: store CC0 preview images when permitted' },
